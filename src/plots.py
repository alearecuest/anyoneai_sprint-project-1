...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    plt.plot(df['date'], df['order_count'], linewidth=1.5, label='Orders per day')
    
    holidays = df[df['holiday'] == True]
    for date in holidays['date']:
        plt.axvline(x=date, color='red', linestyle='--', alpha=0.7, linewidth=1)
    
    plt.title('Orders per Day with Holidays Marked (2017)')
    plt.xlabel('Date')
    plt.ylabel('Number of Orders')
    plt.legend(['Orders', 'Holidays'])
